...
import { useCriticalImagePrefetch } from './hooks/useServiceWorker'
...
export function Header() {
  ...
  return (
    <header 
      ref={headerRef}
      className={`sticky top-0 z-50 bg-white/80 backdrop-blur-sm border-b border-charcoal transition-all duration-500 will-change-transform ${
        isVisible ? 'fade-in' : 'opacity-0 translate-y-[-20px]'
      }`}
    >
          ...
          <div className="flex-grow flex justify-center sm:justify-end">
            <nav className="flex gap-4 sm:gap-6">
              <NavLink 
                to="/" 
                className={({ isActive }) => 
                  `nav-link transition-all duration-300 hover:text-charcoal will-change-transform ${
                    isActive ? 'text-charcoal font-medium' : 'text-gray700'
                  }`
                }
              >
                {t('nav.home')}
